...
static SDL_Surface* loadArtwork(const char* platform_name, int screen_width, int screen_height) {
	...
	// Load pre-cooked artwork from resolution subfolder
	
	char artwork_path[MAX_PATH];
	char resolution_folder[32];
	sprintf(resolution_folder, "%dx%d", screen_width, screen_height);
	
	// Try to load platform-specific pre-cooked artwork
	sprintf(artwork_path, "%s/artworks/%s/%s.png", RES_PATH, resolution_folder, platform_name);
	if (exists(artwork_path)) {
		SDL_Surface* artwork = IMG_Load(artwork_path);
		if (artwork) {
			LOG_info("Loaded artwork: %s\n", artwork_path);
			return artwork; // Already optimized, no scaling needed
		}
	}
	
	// Fall back to default pre-cooked artwork
	sprintf(artwork_path, "%s/artworks/%s/_default.png", RES_PATH, resolution_folder);
	if (exists(artwork_path)) {
		SDL_Surface* artwork = IMG_Load(artwork_path);
		if (artwork) {
	...
	LOG_info("No artwork found for %s at resolution %s\n", platform_name, resolution_folder);
	return NULL;
}
		...
		if (dirty) {
			...
			if (show_main_menu) {
				...
				if (!version) {
					...
					char* extra_key = _("Model");
					char* extra_val = PLAT_getModel(); 
					
					SDL_Surface* release_txt = TTF_RenderUTF8_Blended(font.large, _("Release"), COLOR_DARK_TEXT);
					SDL_Surface* version_txt = TTF_RenderUTF8_Blended(font.large, release, COLOR_WHITE);
					SDL_Surface* commit_txt = TTF_RenderUTF8_Blended(font.large, _("Commit"), COLOR_DARK_TEXT);
				...
				GFX_blitButtonGroup((char*[]){ "B",_("BACK"),  NULL }, 0, screen, 1);
			}
			else {
				// list
				if (total>0) {
					...
					GFX_blitMessage(font.large, _("Empty folder"), screen, &(SDL_Rect){0,0,screen->w,screen->h}); //, NULL);
				}
			
				// buttons
				if (show_setting && !GetHDMI()) GFX_blitHardwareHints(screen, show_setting);
				else if (can_resume) GFX_blitButtonGroup((char*[]){ "X",_("RESUME"),  NULL }, 0, screen, 0);
				else GFX_blitButtonGroup((char*[]){ 
					BTN_SLEEP==BTN_POWER?_("POWER"):_("MENU"),
					BTN_SLEEP==BTN_POWER||simple_mode?_("SLEEP"):_("INFO"),  
					NULL }, 0, screen, 0);
				...
				if (total==0) {
					...
					if (stack->count>1) {
						GFX_blitButtonGroup((char*[]){ "B",_("BACK"), "A",_("OPEN"), NULL }, 1, screen, 1);
					}
					else {
